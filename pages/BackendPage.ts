...
export class BackendPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.editModalFirstName = this.editStudentModal.locator('[name="FIRST_NAME"]');
    this.editModalLastName = this.editStudentModal.locator('[name="LAST_NAME"]');
    this.editModalEmail = this.editStudentModal.locator('[name="EMAIL"]');
    this.editModalDOB = this.editStudentModal.locator('[name="DOB"]');
    this.editModalUpdateButton = this.editStudentModal.locator('button[type="submit"]');
  }
  ...
  async clickEditButtonByName(username: string) {
    await this.getUserByName(username).locator('.common_listControls__2tYWs > svg').first().click()
  }

  async removeUserByName(username: string) {
    await this.getUserByName(username).locator('.common_listControls__2tYWs > svg').last().click()
  }
   ...
   * This function will fill in the provided fields in the dit modal and then click the update button
   * 
   * @param inputs - An object containing the student information
   ...
   * 
   */
  async updateStudent(inputs: { firstName?: string, lastName?: string, email?: string, DOB?: string }) {
    const fieldMap = {
